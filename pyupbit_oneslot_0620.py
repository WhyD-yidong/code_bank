...
def get_open_price(ticker):
    df = pyupbit.get_ohlcv(ticker, count=1, interval="minute60")
    open = df['open']
    last_open = open[-1]
    return last_open
...
while True:
    try:
            ...
            if state is BUY:
                ...
                if sell_price[index] > cur_price:
                    ...
                    if get_balance(crypto_sorted_list[index][0]) < (min_krw / cur_price):
                        ...
                        idx = -1
                        if buy_at_now == 1:
                            buy_price[index] = sell_price[index] * (1 + ratio)
        ...
        #if #(now.hour == 1 or now.hour == 5 or now.hour == 9 or now.hour == 13 or now.hour == 17 or now.hour == 21) and now.minute == 00 and now.second >= 00 and now.second <= 19:
