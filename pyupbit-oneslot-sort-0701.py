    ...
    try:
        ...
        if Trading_start == False:
            ...
            for cnt in range(100):
                get_price = get_buy_price(crypto[cnt]) #매수가
                balance = get_balance(crypto[cnt])
                if get_price * balance > min_krw:
                    state = SELL
                    holding_krw = crypto_krw[cnt]
                    holding = crypto[cnt]
                    holding_ref = get_ref_price(holding_krw)
                    if holding_ref < get_price * (1 - ratio):
                        holding_ref = get_price * (1 - ratio)
                    break
            ...
            if state is BUY:
                ...
                if buy_price[idx] < cur_price and slope[idx] == 1:
                    if krw > min_krw:
                        ...
                        crypto_print(crypto_krw[idx],idx, "T_P:", buy_price[idx], "C_P", cur_price, krw, idx,
                                     "No resource", now.hour, now.minute, now.second)
                else:
                    crypto_print(crypto_krw[idx],idx, "T_P:", buy_price[idx], "C_P:", cur_price, "No satisfy price", format(get_percent(buy_price[idx],cur_price), ".2f"),  now.hour, now.minute, now.second)
                ...
                cur_price = get_current_price(holding_krw)
                cur_balance = get_balance(holding)

                if get_balance(holding) < (min_krw / cur_price):  #pass BUY state
                    state = BUY
                ...
                if holding_ref > cur_price:
                    upbit.sell_market_order(holding_krw, sub_balance)
                    if get_balance(holding) < (min_krw / cur_price):
                        state = BUY
                        if buy_at_now == 1:
                            buy_price[index] = get_price * (1 + ratio)
                            idx = 0
                        crypto_print(holding_krw, holding_ref, cur_price, "Sell out", now.hour, now.minute, now.second)

                else:
                    crypto_print(holding_krw, "T_P:", holding_ref, "C_P:", cur_price, "Over T_P", format(get_percent(holding_ref, cur_price), ".2f"),  now.hour, now.minute, now.second)
                    # please update go to current price over target price during sell.#

        if now.minute == 00 and now.second >= 00 and now.second <= 19:
            if holding_ref < get_price:
                holding_ref = get_price
