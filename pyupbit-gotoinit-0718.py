...
def get_slope(ticker):
    ...
    if ma2[-2] > ma5[-2] and close2[-2] < close2[-1] and close2[-3] < close2[-2]:
        return 1
    else:
        return 0
...
def update_sell_price(index, cur_price):
    ...
    if cur_price > buy_price: #and sell_price[index] < buy_price
...
while True:
    try:
        ...
        if Trading_start == False:
            ...
            for cnt in range(crypto_cnt):
                price15 = get_ma15(crypto_krw[cnt])
                price30 = get_ma30(crypto_krw[cnt])
                price60 = get_ma60(crypto_krw[cnt])
            ...
            if get_price[idx] < cur_price and ma15[idx] < cur_price and ma30[idx] < cur_price and ma60[idx] < cur_price and get_slope(crypto_krw[idx]) == 1:
                ...
                crypto_print(idx, crypto_krw[idx], ma15[idx], ma30[idx], ma60[idx], cur_price, "No satisfy Condition", sell_price[idx], now.hour, now.minute, now.second)
        ...
        if now.minute == 00 and now.second >= 00 and now.second <= 5:
            update_ma60()
            update_ma30()
            update_ma15()

        if now.minute == 30 and now.second >= 00 and now.second <= 5:
            update_ma30()
            update_ma15()

        if (now.minute == 15 or now.minute == 45) and now.second >= 00 and now.second <= 5:
            update_ma15()
