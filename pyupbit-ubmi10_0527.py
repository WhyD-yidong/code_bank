        ...
        if Trading_start == False:
            ...
            if crypto_hold[idx] == 0:
                if ref_price[idx] < cur_price:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "buying condition")
                    if balance_portion > min_krw and krw > min_krw:
                        upbit.buy_market_order(crypto_krw[idx], min(balance_portion, krw) * 0.9995)
                        crypto_hold[idx] = 1
                    else:
                        crypto_print(crypto_krw[idx], "buying - out of condition #1")
                else:
                    crypto_print(crypto_krw[idx], "buying - out of condition #2")

            elif crypto_hold[idx] == 1:
                if ref_price[idx] > cur_price:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "selling condition")
                    if crypto_balance[idx] > (min_krw / cur_price):
                        upbit.sell_market_order(crypto_krw[idx], crypto_balance[idx] * 0.9995)
                        crypto_hold[idx] = 0
                    else:
                        crypto_print(crypto_krw[idx], "selling - out of condition #1")
                else:
                    crypto_print(crypto_krw[idx], "selling - out of condition #2")
