...
while True:
    try:
        ...
        if Trading_start == False:
            ...
            if crypto_hold[idx] == 0:
                if ref_price[idx] < cur_price:
                    if balance_portion > min_krw and krw > min_krw:
                        upbit.buy_market_order(crypto_krw[idx], min(balance_portion, krw) * 0.9995)
			crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "Buy in")
                        crypto_hold[idx] = 1
                    else:
                        crypto_print(crypto_krw[idx], balance_portion, krw, "buying - out of condition #1")
                else:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, "buying - out of condition #2")

            elif crypto_hold[idx] == 1:
                if ref_price[idx] > cur_price:
		    balance = get_balance(crypto[idx])
                    if balance > (min_krw / cur_price):
			upbit.sell_market_order(crypto_krw[idx], crypto_balance[idx] * 0.9995)
			crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "Sell out")
                        crypto_hold[idx] = 0
                    else:
                        crypto_print(crypto_krw[idx], balance, min_krw / cur_price, "selling - out of condition #1")
                else:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, "selling - out of condition #2")
