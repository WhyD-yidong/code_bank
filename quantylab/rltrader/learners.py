...
class ReinforcementLearner:
    ...
    def run(self, learning=True):
        ...
        for epoch in tqdm(range(self.num_epoches)):
            ...
            # 에포크 관련 정보 가시화
            if self.gen_output:
                if self.num_epoches == 1 or (epoch + 1) % max(int(self.num_epoches / 20), 1) == 0:
...
class A2CLearner(ActorCriticLearner):
    ...
    def get_batch(self):
        ...
        for i, (sample, action, value, policy, reward) in enumerate(memory):
            x[i] = sample
            r1 = self.memory_reward[-1] - reward

            #if reward_next < reward: r2 = -1.25
            #elif reward_next > reward: r2 = (reward_next - reward) * 10
            #else: r2 = 0

            r2 = (reward_next - reward) * 100   #gradient on that spot
            #r = reward_next + self.memory_reward[-1] - reward * 2
            reward_next = reward

            y_value[i, :] = value
            y_value[i, action] = np.tanh(r2 + self.discount_factor * value_max_next)
