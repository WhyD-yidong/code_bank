    ...
    try:
        ...
        if Trading_start == False:
            ...
            for index in range(crypto_cnt):
                ...
                limit_price.append(price*(1-ratio))
                crypto_balance.append(balance)
                time.sleep(0.1)

            crypto_print(ref_price)
            crypto_print(limit_price)
            ...
            if ref_price[idx] < cur_price:
                if (balance_portion - balance_on_krw) > min_krw and krw > min_krw:
                    ...
                    crypto_hold[idx] = 1
                    ref_price[idx] = cur_price                    
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "Buy in",now.hour, now.minute, now.second)
                else:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, balance_portion, krw, "buying - out of condition #1",crypto_hold[idx], now.hour, now.minute, now.second)
                    
            elif crypto_hold[idx] == 1:
                if limit_price[idx] > cur_price:
                    if cur_balance > (min_krw / cur_price):
                        upbit.sell_market_order(crypto_krw[idx], cur_balance * 0.9995)
                        ref_price[idx] = (limit_price[idx] + ref_price[idx]) / 2
                        crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "Sell out at limit_price", now.hour, now.minute,
                                 now.second)
                    else:
                        crypto_print(crypto_krw[idx], ref_price[idx], cur_price, cur_balance, min_krw / cur_price,
                                 "selling - out of condition at limit_price",crypto_hold[idx], now.hour, now.minute, now.second)
                else :
                    crypto_print(crypto_krw[idx], cur_price, limit_price[idx],"wait selling at limit_price",crypto_hold[idx], now.hour, now.minute, now.second)                    
                ...
                if cur_balance > (min_krw / cur_price):
                    ...
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, cur_balance, min_krw / cur_price, "selling - out of condition",crypto_hold[idx], now.hour, now.minute, now.second)
            else:
                crypto_print(crypto_krw[idx], ref_price[idx], cur_price, limit_price[idx], "middle price",crypto_hold[idx])
        ...
        if (now.hour == 1 or now.hour == 5 or now.hour == 9 or now.hour == 13 or now.hour == 17 or now.hour == 21) and now.minute == 00 and now.second >= 00 and now.second <= 19: Trading_start = False
        time.sleep(1)
