...
crypto_krw = ["KRW-BTC", "KRW-ETH", "KRW-ADA", "KRW-XRP", "KRW-DOT", "KRW-BCH", "KRW-LINK", "KRW-LTC", "KRW-XLM",
              "KRW-ETC", "KRW-QTUM", "KRW-DOGE"]
        ...
        if Trading_start == False:
            ...
            if ref_price[idx] < cur_price:
                if (balance_portion - balance_on_krw) > min_krw and krw > min_krw:
                    ...
                    limit_price[idx] = cur_price * (1 - ratio)
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "Buy in", now.hour, now.minute,
                                 now.second)
                else:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, balance_portion, krw,
                ...
                if cur_balance > (min_krw / cur_price):
                    ...
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, idx, "Sell out", now.hour, now.minute,
                                 now.second)
                else:
                    crypto_print(crypto_krw[idx], ref_price[idx], cur_price, cur_balance, min_krw / cur_price,
                                 "selling - out of condition", crypto_hold[idx], now.hour, now.minute, now.second)
            else:
                crypto_print(crypto_krw[idx], ref_price[idx], cur_price, limit_price[idx], "middle price",
                             crypto_hold[idx])
