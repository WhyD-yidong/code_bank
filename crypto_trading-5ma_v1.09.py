...
access = "QiWDxozxDC7MMLLtJDjHaJ6gAugjHe1PAdCk53A9"
secret = "B67sCPlxdPsU9vheJAdLeBnNC3sOOJzX5PvQWNQo"
...
simulation = 0
slope_flag = 0
except_list = ["KRW-BTT","KRW-BTC","KRW-ETH"]
...
def get_ticker(idx):
    ...
    #if idx == 1:
    crypto_sorted_list = sorted(crypto_list, key=operator.itemgetter(1), reverse=True)  #decrease
    #else:
    #    crypto_sorted_list = sorted(crypto_list, key=operator.itemgetter(1), reverse=False) #incredase
...
def get_slope_BTC():
    df = pyupbit.get_ohlcv("KRW-BTC", count=22)
    df2 = pyupbit.get_ohlcv("KRW-ETH", count=22)

    ma10 = df['close'].rolling(window=10).mean()
    ma10_2 = df2['close'].rolling(window=10).mean()

    if ma10[-2] < ma10[-1] and ma10_2[-2] < ma10_2[-1]:
        BTC_FLAG = 1
    else:
        BTC_FLAG = 0
    return BTC_FLAG
...
def get_price_condition(ticker):
    ...
    for cnt in range(len(except_list)):
        if ticker == except_list[cnt]:
            return 1, 0
    ...
    if slope_flag == 0:
        if ((df_close[-3] < ma10[-2] and ma10[-2] < df_close[-2]) or (df_close[-3] < ma10[-1] and ma10[-1] < df_close[-2])) and (df_close[-2] < df_close[-3] * 1.2) and (ma5[-2] < ma5[-1]):
            result = 1
            ref = ma10[-2]
        ...
        if ((df_close[-3] < ma10[-2] and ma10[-2] < df_close[-2]) or (df_close[-3] < ma10[-1] and ma10[-1] < df_close[-2])) and (ma5[-2] < ma5[-1]):
            result = 1
            ref = ma10[-2]
        elif (df_close[-3] > ma10[-2] and ma10[-2] > df_close[-2]) or (ma10[-2] > df_close[-3] and df_close[-3] > df_close[-2]):  #rising             
            result = 2
...
def get_price_condition_curr(ticker):
    ...
    ma20 = df['close'].rolling(window=20).mean()
    if slope_flag == 0:
        if ((df_close[-2] < ma10[-1] and ma10[-1] < df_close[-1]) or (df_close[-2] < ma10[-1] and ma10[-1] < df_close[-1])) and (df_close[-1] < df_close[-2] * 1.2):
            result = 1
            ref = ma10[-1]
        ...
        if ((df_close[-2] < ma10[-1] and ma10[-1] < df_close[-1]) or (df_close[-2] < ma10[-1] and ma10[-1] < df_close[-1])):
            result = 1
            ref = ma10[-1]
        elif (df_close[-2] >= ma10[-1] and ma10[-1] >= df_close[-1]) or (ma10[-1] >= df_close[-2] and df_close[-2] >= df_close[-1] and ma10[-2] < df_close[-2]):
            result = 2
...
while True:
    try:
        ...
        if Trading_start == True:
            ...
            crypto_krw, crypto = get_ticker(slope_flag)  # get all item sorted by acc_trading price
            ...
            for cnt in range(crypto_cnt):  #sellout items
                ...
                if cur_balance > 0 and result == 2:                     #sell condition
                    ...
                    if (unit_balance[cnt] * cur_price > min_krw):
                        ...
                        print("{0:3d}:{1}_{2}\t NOT FOR SELL".format(cnt, crypto_krw[cnt], hold[cnt]))
                else:
                    if cur_balance > 0:
                        hold[cnt] = 2000
                        hold_items += 1
            ...
            for cnt in range(crypto_cnt):  #buy in items
                ...
                if hold[cnt] == 1000 and All_result[cnt] == 1 and hold_items < holding_cnt:
                    ...
                    if unit_price > min_krw:
                        ...
                        print("{0:3d}:{1}_{2} Need More Credit".format(cnt, crypto_krw[cnt], hold[cnt]))
                else:
                    if hold[cnt] == 1000 and All_result[cnt] == 1:
                        print("{0:3d}:{1}_{2} Need More Slot".format(cnt, crypto_krw[cnt], hold[cnt]))
                    else:
                        print("{0:3d}:{1}_{2}".format(cnt, crypto_krw[cnt], hold[cnt]))
                time.sleep(0.5)
            ...
            if hold[idx] == 1000 or hold[idx] == 2000 or Total_cnt % 7 != 0:
                ...
                if result == 1:
                    checkin_cnt += 1
                    print(
                        "{0:3d}:{1}_{2}-IN:{3} OUT:{4} SLOT:{5}\t Cur State REF:{6:7.2f}\t cur:{7:7.2f}:({8:3.2f})%\t {9}:{10}:{11}  <==  check in condition".format(
                            idx, crypto_krw[idx], hold[idx], checkin_cnt, checkout_cnt, holding_cnt, price, cur_price, get_percent(price, cur_price), now.hour,
                            now.minute, now.second))
                elif result == 2:
                    checkout_cnt += 1                       
                    print(
                        "{0:3d}:{1}_{2}-IN:{3} OUT:{4} SLOT:{5}\t Cur State REF:{6:7.2f}\t cur:{7:7.2f}:({8:3.2f})%\t {9}:{10}:{11}  <==  check out condition".format(
                            idx, crypto_krw[idx], hold[idx], checkin_cnt, checkout_cnt, holding_cnt, price, cur_price, get_percent(price, cur_price), now.hour,
                            now.minute, now.second))
                else:
                    print("{0:3d}:{1}_{2}-IN:{3} OUT:{4} SLOT:{5}\t Cur State REF:{6:7.2f}\t cur:{7:7.2f}:({8:3.2f})%\t {9}:{10}:{11}".format(idx,
                            crypto_krw[idx], hold[idx], checkin_cnt, checkout_cnt, holding_cnt, price, cur_price, get_percent(price,cur_price), now.hour,
                            now.minute, now.second))
            else:
                if hold[idx] < 2000:
                    hold[idx] += 1
        ...
        if idx >= crypto_cnt:
            ...
            if (checkin_cnt - checkout_cnt) >= 8:
                ...
                if slope_flag == 0:
                    holding_cnt = 3
            print("=== Total count : {0} - {1}:{2} ===".format(Total_cnt,Total_cnt//7,Total_cnt%7))
