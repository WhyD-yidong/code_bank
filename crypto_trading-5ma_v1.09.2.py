...
access = "6sqgQLeNIIT538hheWIY32rQGBCnxpCysTpRX1RG"
secret = "RDu3NFNP0uOJeT3sw26VdGn9T8jRfPZSYsls1Cbp"
...
def get_price_condition(ticker):
    ...
    df = pyupbit.get_ohlcv(ticker, count=32)
    df_close = df['close']
    if len(df_close) < 32:
        return 1, 0
    ...
    if ((df_close[-3] < ma10[-2] and ma10[-2] < df_close[-2]) or (df_close[-3] < ma10[-1] and ma10[-1] < df_close[-2])) and \
        (df_close[-2] < df_close[-3] * 1.2) and (ma5[-3] < ma5[-2]) and (ma10[-2] >= ma30[-2]):
        result = 1
        ref = ma10[-2]
...
def get_price_condition_curr(ticker):
    ...
    if ((df_close[-2] < ma10[-1] and ma10[-1] < df_close[-1]) or (df_close[-2] < ma10[-1] and ma10[-1] < df_close[-1])) and \
        (df_close[-1] < df_close[-2] * 1.2) and (ma5[-2] < ma5[-1]) and (ma10[-1] >= ma30[-1]):
        result = 1
        ref = ma10[-1]
